...
## Follow-up (зафиксировано при мердже тикета 06, не блокер)

- **`GET /utilisation/:id` отсутствует в API**: тикет 05 не добавил tenant-эндпоинт статуса отчёта (поллер доводит SUCCESS/ERROR внутренне). Web (тикет 06) делает поллинг через **идемпотентный повторный POST /utilisation с тем же Idempotency-Key** — возвращает актуальный status. Работает, но для боевой интеграции (и клиентского UI без повторной отправки) добавить `GET /utilisation/:reportId` → `{status, rejectReason}`. НЕ блокер мерджа 06; приоритет — при реальной интеграции ИС МПТ.
