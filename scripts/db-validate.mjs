...
// Uses the shared URL validator (scripts/db-url-validator.mjs) as single source of truth.
...
// Tenant), then drops ONLY the isolated schema. finally-safe: schema is always dropped.
//
...
import { validateTestDatabaseUrl } from "./db-url-validator.mjs";
...
async function main() {
  ...
  try {
    ...
    // finally-safe: drop the schema if it was created but something failed
    if (schemaCreated) {
