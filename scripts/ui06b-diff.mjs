// UI-06b: pixel-diff documents (наш web /documents vs ui-reference.html #documents).
...
async function main() {
  ...
  const protoDocs = await protoShot(browser, "documents");
  ...
  await auth.goto(`${WEB_URL}/documents`, { waitUntil: "networkidle" }).catch(() => {});
  await auth.waitForTimeout(900);
  const webDocs = await auth.screenshot({ fullPage: true });
  await auth.close();
  await browser.close();

  const d = diffBuf(protoDocs, webDocs);
  console.log(`documents diff: ${d.toFixed(2)}%`);
