// W5-02: pixel-diff integrations (наш web /integrations vs ui-reference.html #integrations).
...
async function main() {
  ...
  const proto = await protoShot(browser, "integrations");
  ...
  await auth.goto(`${WEB_URL}/integrations`, { waitUntil: "networkidle" }).catch(() => {});
  await auth.waitForTimeout(900);
  const web = await auth.screenshot({ fullPage: true });
  await auth.close();
  await browser.close();

  const d = diffBuf(proto, web);
  console.log(`integrations diff: ${d.toFixed(2)}%`);
