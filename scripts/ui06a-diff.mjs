// UI-06a: pixel-diff labels (наш web /labels vs ui-reference.html #labels).
...
async function main() {
  ...
  const protoLabels = await protoShot(browser, "labels");
  ...
  await auth.goto(`${WEB_URL}/labels`, { waitUntil: "networkidle" }).catch(() => {});
  await auth.waitForTimeout(900);
  const webLabels = await auth.screenshot({ fullPage: true });
  await auth.close();
  await browser.close();

  const d = diffBuf(protoLabels, webLabels);
  console.log(`labels diff: ${d.toFixed(2)}%`);
