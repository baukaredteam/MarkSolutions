// UI-07: pixel-diff operator + audit (наш web vs ui-reference.html).
...
async function shot(browser, url) {
  ...
  await auth.goto(`${WEB_URL}${url}`, { waitUntil: "networkidle" }).catch(() => {});
  await auth.waitForTimeout(900);
  const buf = await auth.screenshot({ fullPage: true });
  await auth.close();
  return buf;
}
...
async function main() {
  ...
  const protoOperator = await protoShot(browser, "operator");
  const protoAudit = await protoShot(browser, "audit");
  const webOperator = await shot(browser, "/operator");
  const webAudit = await shot(browser, "/audit");
  await browser.close();

  const dOp = diffBuf(protoOperator, webOperator);
  const dAu = diffBuf(protoAudit, webAudit);
  console.log(`operator diff: ${dOp.toFixed(2)}%`);
  console.log(`audit    diff: ${dAu.toFixed(2)}%`);
  const ok = dOp <= 10 && dAu <= 10;
