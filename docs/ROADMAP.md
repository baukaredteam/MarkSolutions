> **Архив OpenCode.** Этот файл — роадмап автономного прототипа (W1–W4 + симулятор ИС МПТ). Канон **реального** контура: [`docs/ROADMAP-REAL.md`](./ROADMAP-REAL.md). Не продолжать W1–W4 / sim как продуктовую цель.
...
**W4 тикет 04 (documents): ЗАКРЫТ** (11.08). Коммит `e2c43ce`: ImportDocument (Q5, unique(tenantId, customsNumber)→409; POST /import: APPLIED-коды заказа → SUCCESS → INTRODUCED-события write-through; ERROR → задача оператору); WithdrawalDocument (Q9: WITHDRAWAL→WITHDRAWN, WRITE_OFF→WRITTEN_OFF; OTHER без comment≥5→400; partialQuantity→400); AggregationUnit/AggregationMember (мин. ADR-025) + childrenWriteOff рекурсивный вывод + DISAGGREGATED; член активного агрегата→409; повторный вывод→409; GET /documents (EntityList: IMPORT|WITHDRAWAL|UTILISATION|SERVICE_ACT_EXPORT) + вкладка на дашборде; симулятор submitImport/submitWithdrawal/getDocument; e2e ввоз+вывод; CONTRACT-IS-MPT.md body детализирован. 165/165 тестов.

**W4 тикет 06 (web-final): ЗАКРЫТ** (11.08). Коммит `65eea50`: GET /dashboard/summary (Q10: codesNotApplied, deadlineSoon от UTIL_DEADLINE_DAYS−7, openAggregates=0 [тикет 03 stretch], docsPendingDt, exceptions [outbox tenant-scoped + UtilisationAlert.firedAt]); UtilisationAlert.firedAt; документы IMPORT|WITHDRAWAL|UTILISATION (общий sort desc, без SERVICE_ACT_EXPORT — тикет 05); POST /demo/w4-seed (идемпотентен: 2 APPLIED-кода, сдвиг updatedAt −24д, EXPECTED-ДТ, outbox FAILED, WithdrawalDocument); веб: summary zero-hidden + deep-links, модалки «Оформить ввоз»/«Вывод»; e2e w4-seed→summary. 169/169 тестов.

**W4 до демо ЗАКРЫТА: тикеты 01+02+04+06. 169/169 тестов. Stretch ПОСЛЕ демо 31.08 — 03 (агрегация SSCC) и 05 (1С-экспорт), их UI/API в MVP НЕ добавляются.**
