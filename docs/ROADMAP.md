...
**T3 тикет 03 (catalog-moderation): ЗАКРЫТ** (07.08). Коммит `…` (feat/t3-moderation): машина модерации CAT-013 (Draft→Validating→Submitted→In Review→Approved/Needs Correction/Rejected→Registering→Registered) с аудитом каждого перехода; GtinResolver трёхслойный (gtin_cache VERIFIED/REJECTED + IG-mod10 PENDING_REAL + ручной ввод source=manual) с флагом REQUIRE_GS1_VERIFIED_FOR_REGISTERING; NktAdapter mock + OutboxPoller (SLA 3с, timeout→FAILED→/moderation/exceptions ID-017, отказ→Needs Correction с field-ошибками); оператор operator@markflow + очередь GET /moderation/queue?tenantId=, approve/reject; reject с fieldReasons→Needs Correction, повторная отправка без исправления = 400; seed gtin_cache RAVENOL/codes_success; web-бейдж «GTIN подтверждён вручную»; оператор без доступа к tenant-данным. Тесты 80/80.
