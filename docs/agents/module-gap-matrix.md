...
| 2 | Центр задач и уведомлений (TASK) | `docs/source/MARK_FLOW_Центр_задач_и_уведомлений_финальный (1).md` | **stub** — `apps/web/src/app.tsx` `STUB_IDS` → `/tasks` (`StubPage`); колокольчик `apps/web/src/layout.tsx` ведёт сюда. Модели `Task` нет (`packages/db/prisma/schema.prisma`). Прокси: `Outbox` FAILED + `UtilisationAlert` в `dashboard.service.ts`; очередь модерации `apps/api/src/moderation.controller.ts`. | P0 | все мутирующие модули; Outbox; ID-017 |
...
2. **Центр задач** — `/tasks` заглушка. Нет сущности задачи/SLA. Исключения размазаны по Outbox + `UtilisationAlert` + кабинет оператора.
