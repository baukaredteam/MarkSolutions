...
| 8   | Операции и документы (OPS)       | `docs/source/MARK_FLOW_Операции_и_документы_детальный_финальный (1).md` | **есть (minimal journal)** — `GET /documents` + alias `GET /operations` = tenant-scoped list import/withdrawal/utilisation (`document.service.list`, без новых типов и без mutating ИС МПТ). UI: `/operations` и `/documents` = `docs.tsx` (не StubPage); empty OPS-28 «Операций пока нет». `utilisation-form.tsx` на `/operations/utilisation`. AT: `apps/api/test/ops-journal-tenant-isolation.spec.ts`. Нет OPS-02…27/29, AT-14, конструктора документов, DT-wizard. | P0       | CONTRACT-IS-MPT `doc/*`; ORD; PRINT; BILL (SETTLE при нанесении); AGG (каскадный вывод) |
...
3. **Операции и документы** — journal slice (OPS-01 list + OPS-28 empty): `/operations` = `docs.tsx`, `GET /operations` alias, utilisation form routed. Нет OPS-02…27/29, нет AT-14.
