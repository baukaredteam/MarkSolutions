...
| **`reconcileOrder` + getOrder / getCodes (post-A4)** | `createOrder` returns STAGE `orderId`; `Order.externalOrderId` persisted; `getOrder`/`getCodes` use `externalOrderId ?? order.id`. `sendToMpt` passes order `productGroup` / `businessPlaceId` when set; adapter default `MPT_PRODUCT_GROUP` = **`autofluids`**. `getOrder` also returns `found`.     | Сверка **до** любого повторного POST. Маппить STAGE `orderId` из ответа create. `getOrder` query `productGroup` — A4 **P1**.                                                             | **P0 closed** (orderId + autofluids + send fields). `releaseMethodType` в теле create — `PRIMARY` default / CONTRACT enum (ORD-releaseMethodType). **Не** STAGE POST.                                                                                                                                                          |
...
Другие факты (не чинить здесь): 4xx excerpt в `MptPermanentError` режет JSON до 200 символов — теоретический риск КМ на utilisation 4xx; `submitImport` / `submitWithdrawal` без Idempotency-Key, operationId у withdrawal = timestamp.
...
- [ ] Зафиксированы: GTIN, qty (**1**), `releaseMethodType` (код шлёт CONTRACT enum, default `PRIMARY`). STAGE POST всё ещё только после «да» Harith.
